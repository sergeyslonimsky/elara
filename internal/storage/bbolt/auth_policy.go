...
// NewPolicyRepo creates a new PolicyRepo backed by the given Manager.
func NewPolicyRepo(manager *Manager) *PolicyRepo {
	return &PolicyRepo{manager: manager}
}
...
func (r *PolicyRepo) RemoveFilteredPolicyCtx(
	ctx context.Context,
	sec, ptype string,
	fieldIndex int,
	fieldValues ...string,
...
func (r *PolicyRepo) LoadPolicyCtx(ctx context.Context, model casbinmodel.Model) error {
...
func (r *PolicyRepo) SavePolicyCtx(ctx context.Context, model casbinmodel.Model) error {
	err := r.manager.Update(ctx, func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(bucketAuthPolicy))
...
func (r *PolicyRepo) AddPolicyCtx(ctx context.Context, sec, ptype string, rule []string) error {
	key, err := ruleKey(sec, ptype, rule)
	if err != nil {
		return err
	}
...
func (r *PolicyRepo) RemovePolicyCtx(ctx context.Context, sec, ptype string, rule []string) error {
	key, err := ruleKey(sec, ptype, rule)
	if err != nil {
		return err
	}
...
func (r *PolicyRepo) ListPermissionsForSubject(ctx context.Context, subject string) ([][]string, error) {
	var rules [][]string

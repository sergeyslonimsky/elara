...
func NewConfigRepo(manager *Manager) *ConfigRepo {
	return &ConfigRepo{manager: manager}
}

// Create creates a new config entry. Writes to content, meta, history, and changelog buckets atomically.
func (r *ConfigRepo) Create(ctx context.Context, cfg *domain.Config) error {
...
func (r *ConfigRepo) Get(ctx context.Context, path, namespace string) (*domain.Config, error) {
...
func (r *ConfigRepo) Update(ctx context.Context, cfg *domain.Config) error {
	if err := r.manager.Update(ctx, func(tx *bolt.Tx) error { return updateConfigTx(tx, cfg) }); err != nil {
...
func (r *ConfigRepo) Delete(ctx context.Context, path, namespace string) (int64, error) {
	var newRev int64
...
func (r *ConfigRepo) CountByNamespace(ctx context.Context, namespace string) (int, error) {
	var count int
...
func (r *ConfigRepo) CurrentRevision(ctx context.Context) (int64, error) {
	var rev int64
...
func (r *ConfigRepo) LockConfig(ctx context.Context, namespace, path string) error {
	err := r.manager.Update(ctx, func(tx *bolt.Tx) error {
		if err := validateNamespaceUnlocked(tx, namespace); err != nil {
			return fmt.Errorf("validate namespace unlocked: %w", err)
		}
...
// Returns ErrLocked if the parent namespace is locked.
func (r *ConfigRepo) UnlockConfig(ctx context.Context, namespace, path string) error {
	err := r.manager.Update(ctx, func(tx *bolt.Tx) error {
		if err := validateNamespaceUnlocked(tx, namespace); err != nil {
			return fmt.Errorf("validate namespace unlocked: %w", err)
		}

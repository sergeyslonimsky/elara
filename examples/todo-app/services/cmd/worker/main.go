// Command worker is a toy background worker. It reads its batch settings from
// Elara over the etcd v3 protocol and reacts to changes live.
package main

import (
	"context"
	"os"
	"os/signal"
	"syscall"

	"github.com/sergeyslonimsky/elara/examples/todo-app/watcher"
)

func main() {
	ctx, stop := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer stop()

	watcher.Run(ctx, watcher.Config{
		Service:  "worker",
		Endpoint: env("ELARA_ENDPOINT", "elara:2379"),
		Key:      env("CONFIG_KEY", "/production/worker/settings.json"),
		Fallback: `{"batch_size":50,"poll_interval_seconds":5}`,
	})
}

func env(key, fallback string) string {
	if v := os.Getenv(key); v != "" {
		return v
	}

	return fallback
}
